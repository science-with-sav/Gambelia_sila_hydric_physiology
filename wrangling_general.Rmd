...
lizards_collared %>% 
  ...
  dplyr::filter(capture_date > "2021-05-01")
...
Sceloporus_study2 <- read_rds("./data/Sceloporus_2021_exp.RDS") %>%
  dplyr::select(date = capture_date, individual_ID, 
                #sex_M_F, mass_G, SVL_mm, # I think I should keep to show species diffs
                CEWL_g_m2h = CEWL_g_m2h_mean,
                msmt_temp_C, msmt_RH_percent)
#Sceloporus_study3 <- 
Sceloporus <- rbind(Sceloporus_study1, Sceloporus_study2) %>%
...
```{r prep Gambelia data}
Gambelia <- lizards_all_SMI %>%
  # get the first measurement we took of each lizard
  group_by(individual_ID) %>%
  mutate(first_date = min(capture_date)) %>%
  dplyr::filter(first_date == capture_date) %>%
  # select the common variables
  dplyr::select(date = capture_date, individual_ID, 
                #sex_M_F, mass_g, SVL_mm,
                CEWL_g_m2h,
                msmt_temp_C, msmt_RH_percent, e_s_kPa, msmt_VPD_kPa) %>%
...
```{r}
write_rds(sp_compare, "./data/species_comparison_df.RDS")
```
