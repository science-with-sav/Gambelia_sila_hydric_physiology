...
# 3-way labels etc
my_labels3 <- c(expression(italic("Gambelia sila")),
               expression(italic("Podarcis erhardii")),
               expression(italic("Sceloporus occidentalis")))
Podar <- brewer.pal(9, "Greens")[c(5)]
my_colors3 <- c(Gambel, Podar, Scelop)
my_shapes3 <- c(21, 24, 23)
```
...
```{r merge 3}
dat_all_liz <- Gambelia %>%
  rbind(Sceloporus) %>%
  dplyr::select(-osmolality_mmol_kg) %>%
  rbind(Podarcis) %>%
  mutate(species = factor(species),
         # calculate VPD based on Campbell & Norman 1998
         e_s_kPa = 0.611 * exp((17.502*msmt_temp_C)/(msmt_temp_C + 240.97)),
...
```{r}
sp3_lm_CEWL <- lm(data = dat_all_liz, CEWL_g_m2h ~ species)
CEWL_emmean3 <- data.frame(emmeans(sp3_lm_CEWL, "species")) %>%
  rename(CEWL_emmean = emmean,
         CEWL_lower = lower.CL,
         CEWL_upper = upper.CL)
pairs(emmeans(sp3_lm_CEWL, "species"))
```
...
```{r}
sp3_lm_msmt_temp <- lm(data = dat_all_liz, msmt_temp_C ~ species)
temp_emmean3 <- data.frame(emmeans(sp3_lm_msmt_temp, "species")) %>%
  rename(temp_emmean = emmean,
         temp_lower = lower.CL,
         temp_upper = upper.CL)
pairs(emmeans(sp3_lm_msmt_temp, "species"))

CEWL_temp_emmeans3 <- temp_emmean3 %>%
  left_join(CEWL_emmean3, by = 'species')
CEWL_temp_emmeans3
```
...
```{r}
sp3_lm_msmt_VPD <- lm(data = dat_all_liz, msmt_VPD_kPa ~ species)
VPD_emmean3 <- data.frame(emmeans(sp3_lm_msmt_VPD, "species")) %>%
  rename(VPD_emmean = emmean,
         VPD_lower = lower.CL,
         VPD_upper = upper.CL)
pairs(emmeans(sp3_lm_msmt_VPD, "species"))

CEWL_VPD_emmeans3 <- VPD_emmean3 %>%
  left_join(CEWL_emmean3, by = 'species')
CEWL_VPD_emmeans3
```
...
ggplot() +
  geom_point(data = dat_all_liz,
      ...
      alpha = 0.3,
      size = 0.9) +
  geom_point(data = CEWL_temp_emmeans3,
             aes(x = temp_emmean,
                 y = CEWL_emmean,
                 shape = species,
                 fill = species),
             ...
             alpha = 1,
             color = "black") +
  scale_color_manual(values = my_colors3, name = NULL, labels = my_labels3) +
  scale_fill_manual(values = my_colors3, name = NULL, labels = my_labels3) +
  scale_shape_manual(values = my_shapes3, name = NULL, labels = my_labels3) +
  #scale_x_continuous(limits = c(1.37,4.32)) +
  scale_y_continuous(limits = c(0,35)) +
  labs(x = "Temperature (°C)",
       y = bquote('CEWL (g '*m^-2*' '*h^-1*')')) +
  savs_ggplot_theme -> CEWL_temp_check3
CEWL_temp_check3
```
...
ggplot() +
  geom_point(data = dat_all_liz,
      ...
      alpha = 0.3,
      size = 0.9) +
  geom_point(data = CEWL_VPD_emmeans3,
             aes(x = VPD_emmean,
                 y = CEWL_emmean,
                 shape = species,
                 fill = species),
  ...
  scale_fill_manual(values = my_colors3, name = NULL, labels = my_labels3) +
  scale_shape_manual(values = my_shapes3, name = NULL, labels = my_labels3) +
  scale_x_continuous(limits = c(0.8,4.32)) +
  scale_y_continuous(limits = c(0,35)) +
  labs(x = "VPD (kPa)",
       y = bquote('CEWL (g '*m^-2*' '*h^-1*')')) +
  savs_ggplot_theme -> CEWL_VPD_check3
CEWL_VPD_check3
```
...
## Export Grouped 3-Species Figure


```{r}
ggarrange(CEWL_temp_check3, CEWL_VPD_check3,
          ncol = 1, nrow = 2,
          labels = c("A", "B"),
          label.y = 1.015, label.x = 0.03,
          common.legend = TRUE,
          legend = "bottom"
          ) -> sp3_compare_2panel_fig

ggsave(filename = "species3_compare.pdf",
       plot = sp3_compare_2panel_fig,
       path = "./results_figures",
       device = "pdf",
       dpi = 600,
       units = "mm",
