...
**I need to do this with April data only!**
...
- SMI / osml
- SMI / hct
- cloacal temp / capture to msmst time
...
osml_sex_mod <- (lmerTest::lmer(data = lizards_over_time, 
                       osmolality_mmol_kg ~ time_pt + sex_M_F + (1|individual_ID)))
...
Already did model above, so just plot:
...
ggplot(data = lizards_over_time) + 
  geom_point(aes(x = cloacal_temp_C, 
               ...
               size = 2,
               alpha = 1) +
  #geom_line(aes(x = cloacal_temp_C, 
   #                y = CEWL_g_m2h, 
    #            group = individual_ID,
     #           color = individual_ID),
      #      alpha = 0.8) +
