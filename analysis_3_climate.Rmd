...
sunrises_full <- rise_April %>%
  rbind(rise_May) %>%
  rbind(rise_June) %>%
  rbind(rise_July) %>%
  dplyr::select(date, time) 
sunrises_formatted <- sunrises_full %>%
  mutate(sunrise = as.POSIXct(paste(date, time, sep = " ")),
         date = as.Date(date)) %>%
  dplyr::select(-time)
...
sunsets_full <- set_April %>%
  rbind(set_May) %>%
  rbind(set_June) %>%
  rbind(set_July) %>%
  dplyr::select(date, time) 
sunsets_formatted <- sunsets_full %>%
  mutate(sunset = as.POSIXct(paste(date, time, sep = " ")),
         date = as.Date(date)) %>%
  dplyr::select(-time)
...
sunsets_formatted_2018 <- sunsets_full %>%
  mutate(date = paste("2018", substr(date, 6, 10), sep = "-"),
         sunset = as.POSIXct(paste(date, time, sep = " ")),
         date = as.Date(date)) %>%
  dplyr::select(-time)
sunsets_formatted_2019 <- sunsets_full %>%
  mutate(date = paste("2019", substr(date, 6, 10), sep = "-"),
         sunset = as.POSIXct(paste(date, time, sep = " ")),
         date = as.Date(date)) %>%
  dplyr::select(-time)
sunsets_formatted_2020 <- sunsets_full %>%
  mutate(date = paste("2020", substr(date, 6, 10), sep = "-"),
         sunset = as.POSIXct(paste(date, time, sep = " ")),
         date = as.Date(date)) %>%
  dplyr::select(-time)
...
annual_climate_rainfall <- cum_rain %>%
  ...
  mutate(May_prop_below = c(0.38, 0.25, NA, 0.2),
         June_prop_below = c(0.65, 0.31, NA, 0.5),
         July_prop_below = c(0.81, 0.43, NA, 0.8))
...
```{r}
write_csv(annual_climate_rainfall, "./results_statistics/climate_across_years.csv")
```
