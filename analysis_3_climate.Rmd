...
*Note: the burrow data comes from a much larger dataset, which I wrangle/prep in another Rproject/guthub repo*
...
Throughout the time that I had HOBOs deployed in burrows, several loggers were chewed by mammals (probe broken), burrows were back-filled by kangaroo rats (and probes buried), or one burrow was completely excavated by kit foxes (probe was then logging aboveground microclimate). We will remove this data so that we only keep measurements taken from burrows that Blunt-nosed Leopard Lizards would typically be seen taking refuge in. 

Check values for HOBOs: C, D, E, and L (*I didn't create unique code for each thing*)

**HOBO C** was back-filled a few times when I checked on it, but both temp and RH looked totally fine/normal, so I'll keep all the data for that logger. 

**HOBO D** was back-filled a few times, but the temp and RH actually looked very erratic. I will get rid of that data completely.

**HOBOs L and E** both broke mid-season, and were replaced June 10. To be safe, I will only keep the data for them after being re-deployed. 


```{r}
dat_HOBO %>% 
  dplyr::filter(HOBO_ID == "D") %>% 
  ggplot(aes(
         x = date_time,
         y = RH_percent,
         color = HOBO_ID
...
dat_HOBO %>% 
  ...
  geom_point() +
  geom_vline(xintercept = as.POSIXct("2021-06-10 20:00"))
```
...
- get rid of HOBO D data completely
- remove HOBO E and L data up to June 10

- notes: 
  - HOBO C removal = 731 rows
  - HOBO E removal = 944 rows
  - HOBO L removal = 278 rows


```{r}
dat_HOBO_clean <- dat_HOBO %>% 
  dplyr::filter(HOBO_ID != "C") %>% 
  dplyr::filter(!(HOBO_ID == "L" & date_time < as.POSIXct("2021-06-10 20:00"))) %>% 
  dplyr::filter(!(HOBO_ID == "E" & date_time < as.POSIXct("2021-06-10 20:00")))

# check
nrow(dat_HOBO_clean) + 731 + 944 + 278 == nrow(dat_HOBO)
```
...
```{r}
dat_HOBO_means_by_burrow <- dat_HOBO_clean %>%
...
and then, means across burrows for each time chunk:

```{r}
dat_HOBO_chunk_means <- dat_HOBO_chunks_by_burrow %>%
  # get avg across days within a time periods for each daytime and nighttime 
  group_by(date_chunks, daytime
           ) %>%
  # get the avg daily mean, min, and max per time period
  summarise(temp_C_chunk_mean = mean(temp_C_chunk_mean_by_HOBO),
            temp_C_chunk_min = min(temp_C_chunk_mean_by_HOBO),
            temp_C_chunk_max = max(temp_C_chunk_mean_by_HOBO),
            RH_chunk_mean = mean(RH_chunk_mean_by_HOBO),
            RH_chunk_min = min(RH_chunk_mean_by_HOBO),
            RH_chunk_max = max(RH_chunk_mean_by_HOBO),
            VPD_kPa_chunk_mean = mean(VPD_kPa_chunk_mean_by_HOBO),
            VPD_kPa_chunk_min = min(VPD_kPa_chunk_mean_by_HOBO),
            VPD_kPa_chunk_max = max(VPD_kPa_chunk_mean_by_HOBO),
            n = n()
            ) %>% 
  mutate(logger = "Burrow")

summary(dat_HOBO_chunk_means)
```
...
dat_ws_chunk_means <- dat_ws_means %>%
  ...
  summarise(temp_C_chunk_mean = mean(temp_C_daily_mean),
            temp_C_chunk_min = min(temp_C_daily_mean),
            temp_C_chunk_max = max(temp_C_daily_mean),
            RH_chunk_mean = mean(RH_daily_mean),
            RH_chunk_min = min(RH_daily_mean),
            RH_chunk_max = max(RH_daily_mean),
            VPD_kPa_chunk_mean = mean(VPD_kPa_daily_mean),
            VPD_kPa_chunk_min = min(VPD_kPa_daily_mean),
            VPD_kPa_chunk_max = max(VPD_kPa_daily_mean),
            wind_speed_mph_chunk_mean = mean(wind_mph_mean),
            n = n()
            ) %>% 
  mutate(logger = "Aboveground")

summary(dat_ws_chunk_means)
```
...
```{r}
all_clim_dat <- dat_ws_chunk_means %>% 
  dplyr::select(-wind_speed_mph_chunk_mean) %>% 
  rbind(dat_HOBO_chunk_means) %>% 
  mutate(logger_daytime = factor(paste(logger, daytime, sep = " "),
                         levels = c("Burrow Day",
                                    "Aboveground Day",
                                    "Burrow Night",
                                    "Aboveground Night")),
...
HOBO_temp_emmean <- data.frame(emmeans(HOBO_temp_lm, 
                                       pairwise ~ daytime | date_chunks)$emmean) %>% 
  #data.frame(emmeans(HOBO_temp_lm, "date_chunks")) %>%
  # add reference for plots
  mutate(type = "Burrow")

HOBO_temp_pairwise <- data.frame(emmeans(HOBO_temp_lm, 
                                       pairwise ~ daytime | date_chunks)$contrasts)
  #pairs(emmeans(HOBO_temp_lm, "date_chunks"))
HOBO_temp_pairwise
```
...
```{r}
ggplot(data = all_clim_dat) +
  geom_pointrange(
    aes(
      x = (date_chunks),
      y = temp_C_chunk_mean,
      ymin = temp_C_chunk_min,
      ymax = temp_C_chunk_max,
      group = logger_daytime,
      color = logger_daytime,
      fill = logger_daytime,
      shape = logger_daytime
    ),
  ...
  geom_vline(xintercept = 7.5,
             ...
             size = 0.5) +
  # pretty-fy
  scale_fill_manual(values = c(brewer.pal(11, "RdYlBu")[c(8, 4, 11, 1)]), name = NULL) +
  scale_shape_manual(values = c(21, 23, 21, 23), name = NULL) +
  scale_color_manual(values = c(brewer.pal(11, "RdYlBu")[c(10, 2, 11, 1)]), name = NULL) +
  # additional formatting
  ylim(8, 40) +
  # guides(shape = guide_legend(nrow = 2),
  #        fill = guide_legend(nrow = 2)) +
  savs_ggplot_theme +
  theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 5, 
                             unit = "pt"),
...
ggplot(data = all_clim_dat) +
  ...
  #        fill = guide_legend(nrow = 2)) +
  savs_ggplot_theme +
  theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 0.5, 
                             unit = "pt"),
        legend.margin = margin(0,0,0,0, unit = "pt")
        ) -> daily_RH_fig
daily_RH_fig
...
dat_HOBO_chunks_by_burrow %>% 
  dplyr::filter(RH_chunk_mean_by_HOBO == 0)
```
...
ggplot(data = all_clim_dat) +
  geom_pointrange(
    aes(
      ...
      ymin = VPD_kPa_chunk_min,
      ymax = VPD_kPa_chunk_max,
      group = logger_daytime,
      color = logger_daytime,
      fill = logger_daytime,
      shape = logger_daytime
  ...
  #        fill = guide_legend(nrow = 2)) +
  savs_ggplot_theme +
  theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 9.2, 
                             unit = "pt"),
        legend.margin = margin(0,0,0,0, unit = "pt")
        ) -> daily_VPD_fig
daily_VPD_fig
