...
micro_hydric <- liz_dat_apr_may_only %>%
  left_join(prop_above, by = c('date_chunks', 
                              'radio_collar_serial' = 'Serial')) %>%
  # remove NAs
...
There are basically **no detectable relationships** between water balance and thermoregulation or microhabitat use in BNLL. The only significant relationship we found was between plasma osmolality and relative date of onset of estivation, but the effect size was miniscule. 

*Do I use actual microhabitat or microhabitat PROPORTION as the response?*
if actual microhabitat, I think I need to use full, raw dataset.
...
```{r}
MH_use_above_or_no <- MH_use_across_indivs_sex %>%
  mutate(aboveground = factor(case_when(microhabitat == "Burrow" ~ "Belowground",
                                        microhabitat != "Burrow" ~ "Aboveground"),
                              levels = c("Aboveground", "Belowground")))
summary(MH_use_above_or_no)
```
...
summary(glm(data = MH_use_above_or_no, aboveground ~ date_chunks,
            family="binomial"))
```
...
MH_use_across_indivs_sex %>%
  dplyr::filter(microhabitat == "Burrow") %>%
  lm()

(lm(data = MH_use_across_indivs_sex, ))
