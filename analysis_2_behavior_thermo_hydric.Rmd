...
my_brew_cols_4a <- brewer.pal(12, "Paired")[c(7,8,1,2)]
my_brew_cols_4b <- brewer.pal(11, "Spectral")[c(2,4,9,11)]
```
...
First, we compute maximum temperatures (thermal metrics) and microhabitat use for each lizard at different time points, then we pair that with our hydric physiology data, and look for any relationships.
...
Subset the temperature data so that we do not use the temperatures from during hydric measurement periods. Also add columns for time chunks to make averaging easier later.
...
Sunrise during our study: 6:05-6:40 am --\> middle \~\~6:20? Sunset during our study: 20:24-21:07 (8:24-9:07 pm) --\> middle \~\~20:40? (entire study during DST) from [NOAA solar calculator](https://gml.noaa.gov/grad/solcalc/)
...
get monthly values for comparison across BNLL study years:

```{r}
MH_use_across_indivs_MONTH <- microhab %>%
  mutate(month = factor(substr(as.character(date_only), 6,7))) %>%
  group_by(month, microhabitat) %>%
  summarise(n_obs_micro = n()) %>%
  group_by(month) %>%
  mutate(n_obs_total = sum(n_obs_micro)) %>%
  ungroup() %>%
...
#anova(max_temp_CEWL_mod, type = "1", ddf = "Kenward-Roger")
...
#anova(max_temp_osml_mod, type = "1", ddf = "Kenward-Roger")
plot(max_temp_osml_mod)
simple.eda(residuals(max_temp_osml_mod))
...
#anova(max_temp_mass_mod, type = "1", ddf = "Kenward-Roger")
...
#anova(max_temp_SMI_mod, type = "1", ddf = "Kenward-Roger")
...
summary(db_osml_mod)
plot(db_osml_mod)
simple.eda(residuals((db_osml_mod)))
...
```{r}
anova(max_temp_CEWL_mod, type = "1", ddf = "Kenward-Roger")
anova(db_CEWL_mod, type = "1", ddf = "Kenward-Roger")
anova(micro_CEWL_mod, type = "1", ddf = "Kenward-Roger")
```
...
anova(max_temp_osml_mod, type = "1", ddf = "Kenward-Roger")
#summary(max_temp_osml_mod)
anova(db_osml_mod, type = "1", ddf = "Kenward-Roger")
#summary(db_osml_mod)
anova(micro_osml_mod, type = "1", ddf = "Kenward-Roger")
```
...
```{r}
anova(max_temp_mass_mod, type = "1", ddf = "Kenward-Roger")
anova(db_mass_mod, type = "1", ddf = "Kenward-Roger")
anova(micro_mass_mod, type = "1", ddf = "Kenward-Roger")
```
...
anova(max_temp_SMI_mod, type = "1", ddf = "Kenward-Roger")
#summary(max_temp_SMI_mod)
anova(db_SMI_mod, type = "1", ddf = "Kenward-Roger")
anova(micro_SMI_mod, type = "1", ddf = "Kenward-Roger")
```
...
corrs <- data.frame(hydric_var = c(rep("CEWL", 3), rep("Plasma Osmolality", 3), 
                                   rep("Body Mass", 3), rep("Body Condition", 3)),
                    thermal_var = c(rep(c("Max Temp", "Db", "Micro Use"), 4)),
                    R = c(
                      # CEWL
...
also look at just July for hydric \~ estivating or not:

```{r}
est_hydric_July <- liz_dat_july_only %>%
  left_join(estivating, by = c('radio_collar_serial' = 'Serial')) %>%
  mutate(est_or_not = case_when(is.na(est_onset) == TRUE ~ "Active",
                                is.na(est_onset) == FALSE ~ "Estivating"))
est_hydric_Apr_may <- liz_dat_apr_may_only %>%
  group_by(radio_collar_serial) %>%
  summarise(mean_mass = mean(mass_g),
            mean_SMI = mean(SMI),
            mean_CEWL = mean(CEWL_g_m2h),
  ...
  left_join(est_hydric_July, by = 'radio_collar_serial')
```
...
# mass
est_mass_mod <- lm(data = est_hydric, est_onset_scaled ~ mean_mass)
summary(est_mass_mod)
anova(est_mass_mod)
plot(est_mass_mod)

# SMI
est_SMI_mod <- lm(data = est_hydric, est_onset_scaled ~ mean_SMI)
summary(est_SMI_mod)
anova(est_SMI_mod)
plot(est_SMI_mod)
```
...
compare boxplots of the same individuals in April/May (avg) vs July to make sure we have no outliers, and if no outliers, what trends look like:

first, have to make the data:

```{r}
liz_dat_apr_only <- liz_dat_apr_may_only %>%
  dplyr::filter(individual_ID %in% est_hydric_July$individual_ID) %>%
  ...
  left_join(est_hydric_July, by = 'individual_ID') %>%
  dplyr::filter(individual_ID != "F-10")
```
...
```{r}
# CEWL
ggplot(liz_dat_apr_only) +
  aes(x = est_or_not,
      y = mean_CEWL,
      color = individual_ID) +
  geom_point()
ggplot(est_hydric_July) +
  aes(x = est_or_not,
      y = CEWL_g_m2h,
      color = individual_ID) +
  geom_point()
...
I'm worried about the distribution of the data for the active lizards.

```{r}
est_hydric_July %>%
  summarise(mean(CEWL_g_m2h), sd(CEWL_g_m2h),
            mean(osmolality_mmol_kg_mean), sd(osmolality_mmol_kg_mean),
...
It looks like F-10 is quite far outside the distribution of other points for mass and SMI, and would be likely to affect statistics, so remove her data:

```{r}
est_hydric_July2 <- est_hydric_July %>%
  dplyr::filter(individual_ID != "F-10")
```
...
```{r}
# CEWL
est_CEWL_JULY_mod <- lm(data = est_hydric_July, 
                        CEWL_g_m2h ~ est_or_not)
anova(est_CEWL_JULY_mod)

# osmolality
est_osml_JULY_mod <- lm(data = est_hydric_July, 
                        osmolality_mmol_kg_mean ~ est_or_not)
anova(est_osml_JULY_mod)

# mass
est_mass_JULY_mod <- lm(data = est_hydric_July2, 
                        mass_g ~ est_or_not)
anova(est_mass_JULY_mod)

# SMI
est_SMI_JULY_mod <- lm(data = est_hydric_July2, 
                        SMI ~ est_or_not)
anova(est_SMI_JULY_mod)
```

What were those lizards like in April? Do the same models for April/May data:

```{r}
# CEWL
est_CEWL_April_mod <- lm(data = liz_dat_apr_only, 
                        mean_CEWL ~ est_or_not)
anova(est_CEWL_April_mod)

# osmolality
est_osml_April_mod <- lm(data = liz_dat_apr_only, 
                        mean_osml ~ est_or_not)
anova(est_osml_April_mod)

# mass
est_mass_April_mod <- lm(data = liz_dat_apr_only, 
                        mean_mass ~ est_or_not)
anova(est_mass_April_mod)

# SMI
est_SMI_April_mod <- lm(data = liz_dat_apr_only, 
                        mean_SMI ~ est_or_not)
anova(est_SMI_April_mod)
```

Seems like no carry-over differences from April into July, which is good!
...
car::Anova(micro_glmm_sex, type = 2, test.statistic = "Chisq", error.estimate = "pearson")
...
ggplot(MH_use_across_indivs) + 
    geom_bar(aes(x = date_chunks,
             ...
             position = "fill", 
             stat = "identity") +
  scale_fill_manual(values = my_brew_cols_4a) +
  ggtitle("brew4a") +
  savs_ggplot_theme  -> b4a
...
ggplot(MH_use_across_indivs) + 
    geom_bar(aes(x = date_chunks,
             ...
             position = "fill", 
             stat = "identity") +
  scale_fill_manual(values = my_brew_cols_4b) +
  ggtitle("brew4b") +
  savs_ggplot_theme  -> b4b



ggarrange(v1, v2, v3, v4, v5, b4, b4a, b4b,
          ncol = 2, nrow = 4,
...
Notes on measurements of microhabitat use:
