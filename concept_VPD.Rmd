...
Vapor pressure deficit (VPD) is the drying power of the air, and it's a function of both temperature and water vapor. We believe VPD is the driver of plasticity in skin permeability to water loss. However, it is uncommon in studies of organismal physiology, so in this document, we create several figures to help explain the concept. 
...
and create some comparison data:


```{r}
dat_es_VPD_compare <- dat_100_only %>% 
  mutate(divide = 2.336479,
         minimum = 0)
```
...
ggplot() + 
  ...
  # axis labels
  scale_x_continuous(limits = c(20, 41),
  ...
  # line labels
  annotate(geom = "text", parse = F, x = 40.5, y = 3, 
           label = "40% RH", hjust = 0) +
  annotate(geom = "text", parse = F, x = 40.5, y = 7.43, 
           label = "100% RH", hjust = 0) +
...
ggplot() + 
  ...
  geom_ribbon(data = dat_es_VPD_compare,
              aes(x = temp_C,
                  ymin = minimum,
                  ymax = divide),
              fill = "deepskyblue3",
  ...
  # divide
  geom_hline(yintercept = 2.336479, size = 1, color = "black", linetype = "dashed") +
  ...
  theme(
    legend.position = "none"
  ) -> VP_D_compare_fig_water

VP_D_compare_fig_water

ggsave(filename = "concept_VPD_overall_water.pdf",
       plot = VP_D_compare_fig_water,
       path = "./results_figures",
       device = "pdf",
       dpi = 600,
       units = "mm",
...
```{r}
VP_D_compare_fig <- VP_D_compare_fig_water + 
  ...
  # 100% RH only
  geom_smooth(data = dat_es_VPD_compare,
              aes(x = temp_C,
                  y = e_a_kPa),
              method = "loess",
              formula = y~x,
  ...
  # ribbon labels
  #annotate(geom = "text", parse = F, x = 30, y = 1.15, label = "Ambient") +
  annotate(geom = "text", parse = F, x = 35, y = 4, 
           label = "Vapor Pressure Deficit")
  # annotate(geom = "text", parse = F, x = 30, y = 4.6, angle = 30,
  #          label = "Saturation") 
  

VP_D_compare_fig

ggsave(filename = "concept_VPD_overall.pdf",
       plot = VP_D_compare_fig,
       path = "./results_figures",
       device = "pdf",
       dpi = 600,
       units = "mm",
